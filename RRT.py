...
class RRT:
    ...
    def normal(self,c_min):
        nor = (self.goal.row-self.start.row)/c_min, (self.goal.col-self.start.col)/c_min 
        return nor 
    
    def rtow(self,c_min):
        
        #scaling factor for the rotation matrix
        
        scal_fact = self.normal(c_min)
        ...
        #U(sigma)V = M 
        M = []
        for i in range(len(scal_fact)):
            M.append([scal_fact[i], I1[i]])
        M = np.array(M)
        #Using Singular Value Decomposition 
        #S is diagonal matrix which is ignored
        U, _, V = np.linalg.svd(M)
        determinant = np.linalg.det(U) * np.linalg.det(V)
        d_mat = np.diag([1, determinant])

        #Rotation matrix C is given by:
        C = U @ d_mat @ V.T
        return C
    
    def center(self):
       C = [(self.start.row+self.goal.row)/2, (self.start.col+self.goal.col)/2]
       return C
   
    def unitball(self):
        pointx = np.random.normal(0, 1)
        pointy = np.random.normal(0,1)
        point = (pointx,pointy)
        return point
    ...
    def sample(self, goal_bias=0.05, c_best=0):
        ...
        if c_best > 0:
            point = self.get_new_point_in_ellipsoid(goal_bias, c_best)
